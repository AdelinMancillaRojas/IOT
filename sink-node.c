...
PROCESS(sink_node,"Sink Node"); expected an __is_identifier
AUTOSTART_PROCESSES(&sink_node);
static void 
rx_callback(struct simple_upd_connection *client_connection,
...
uint16_t data_length){
    ...
    LOG_INFO("Received packet %u from node ",counter);
...
PROCESS_THREAD(sink_node,ev,data)
{
    ...
    PROCESS_BEGIN();
    NETSTACK ROUTIN. root_start();
    simple_udp_register(&connection, SERVER PORT, NULL,CLIENT_PORT, rx_callback);
